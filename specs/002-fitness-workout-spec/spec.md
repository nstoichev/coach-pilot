...
8a. **Given** a metric exercise that offers a Custom measure option (e.g. Row, or Run in the future), **When** the user selects Custom, **Then** a text field is shown instead of a range slider and the user can enter free-form text (e.g. \"1 mile\", \"10 km\"); **When** the user tries to complete the workout with empty custom text, **Then** validation blocks completion; **When** the user opens Advanced settings with Custom selected, **Then** speed/watts (or other advanced metrics) remain available and are stored with the custom measure.
...
3a. **Given** the Workout Board is displayed and at least one assigned exercise has non-empty equipment metadata, **Then** a section titled **"Equipment required"** appears **below** all segments (after the segment/rest layout), listing each required equipment item **once**, in a deterministic sorted order, with labels normalized per aggregation rules (trim, case-insensitive deduplication). **Given** no assigned exercise contributes a non-empty equipment label, **Then** this section is **not** shown.
...
- **FR-012**: System MUST allow metric-style exercises (for example row, bike, or run) to store metric targets such as calories, distance, speed, time, or a **custom** free-text measure (e.g. \"1 mile\", \"10 km\") instead of sets/repetitions.
...
- **FR-021**: Assigned-exercise sets MUST use a range slider from 0 to 10, step 1 (when shown; see FR-020a); reps MUST use a range slider from 1 to 50, step 1. Metric type (calories, distance, speed, time, or **custom**) MUST be chosen via the shared **`SegmentedControl`** pattern (mutually exclusive segments), styled only with **semantic design tokens** from `tailwind.config.js` as required by the project constitution — **no** default Tailwind palette utilities (e.g. `gray-*`, `blue-*`) and **no** ad-hoc CSS classes unless constitution-approved. For non-custom types, metric value MUST use a range slider with type-specific min, max, and step (e.g. calories 0–500 step 5, distance in m, time in seconds with 15 s steps). For **custom** type, the UI MUST show a single text field (no range slider) where the user can enter free-form text (e.g. \"1 mile\", \"10 kilometers\"); validation MUST require non-empty custom text before save. For calories and distance only, a \"Max\" toggle MUST be available next to the metric value label; when enabled, the metric target is interpreted as \"max\" effort until the segment time ends and the value slider is hidden. For all sets-reps exercises, a \"Max reps\" toggle MUST be available next to the reps label; when enabled, the reps slider is hidden and the assignment is interpreted as \"max reps until the segment time ends\".
- **FR-021a**: For metric exercises that support optional advanced metrics (e.g. Row, Run), speed and similar measures that cannot be structured as a completable target MUST NOT be primary measure options. Such exercises MAY declare `advancedMetrics` (e.g. speed, watts). The UI MUST show an \"Advanced settings\" button at the bottom of the exercise prescription; when clicked, it MUST reveal additional range sliders (e.g. speed, watts) that are hidden by default to avoid overloading the UI. These advanced values are optional and are stored on the assignment when set. **Custom** measure MAY be offered alongside calories, distance, time for metric exercises (e.g. Row, Run); when custom is selected, the user enters free text and advanced settings remain available.
...
- **FR-028**: The builder MUST apply **distinct visual states** to segment cards so the user can see at a glance which segments are complete and which are incomplete: segments **with at least one exercise** MUST use **success** and/or **accent** semantic tokens (e.g. `border-success`, `bg-success-soft`, `text-text-primary` on appropriate surfaces) so the state reads as “has exercises”; segments **with no exercises** MUST use **danger** and/or **text-muted** with **surface** tokens (e.g. `border-danger`, `bg-surface-secondary`, `text-text-muted`) so the state reads as incomplete or warning. Implementation MUST use only Tailwind utilities backed by tokens in `tailwind.config.js`, per the project constitution. The two states MUST remain clearly distinguishable.
- **FR-031**: The Workout Board MUST derive and display **required equipment** from all assigned exercises’ optional `equipment` lists: the UI MUST show a read-only **"Equipment required"** block **below** the segment list when the derived list is non-empty; labels MUST be trimmed, deduplicated case-insensitively (each distinct item once), and sorted deterministically (see `specs/002-fitness-workout-spec/research.md` Decision 6). Aggregation logic MUST live in a deterministic service function (e.g. `getWorkoutRequiredEquipment`), not ad hoc in the component. When the derived list is empty, the board MUST NOT show an equipment section.
