...
This feature establishes the **Workout Builder** and **Exercise Database** foundation for the Coach Pilot fitness application. It solves the problem of planning and reusing structured workouts made of segments and exercises, and it defines the domain model that later features (Timer Generator, Fatigue System, equipment aggregation) will consume.
...
- **Place in the app**: Workout Builder (create/edit workouts and segments); Exercise Database (create/edit/delete exercises). Placeholders are set for Timer Generator, Fatigue System, and workout auto-generation.
...
2. **Given** a segment exists, **When** the user adds exercises from the Exercise Database, **Then** only valid exercises are added and shown under that segment.
3. **Given** a workout has segments and exercises, **When** the user reorders items, **Then** the new order is preserved and reflected immediately.

---

### User Story 2 — Manage Exercise Database (Priority: P2)

As a coach or trainee, I can add, edit, and delete exercises with required and optional attributes so workouts can be composed from accurate exercise definitions.

**Why this priority**: The Workout Builder depends on reliable exercise data; the Exercise Database enables reuse and reduces setup time for future workouts and for the Fatigue System and equipment aggregation.
...
1. **Given** the Exercise Database is open, **When** the user creates an exercise with required fields, **Then** the exercise becomes selectable in segment assignment.
2. **Given** an exercise exists, **When** the user updates optional fields (equipment, muscles, working weight), **Then** the changes are visible wherever the exercise is referenced.
3. **Given** an exercise is referenced by a segment, **When** the user attempts to remove it, **Then** the system prevents accidental invalid workout state and prompts corrective action.
...
- **FR-007**: System MUST model segments with identity, name, and an ordered list of exercises.
- **FR-008**: System MUST model workouts with identity, name, ordered segments, and optional rest-between-segments (e.g. in minutes).
- **FR-009**: Users MUST be able to add, edit, and delete exercises from the Exercise Database; exercise records MUST be the single source of truth for workout-building flows.
- **FR-010**: System MUST support extension points so the Timer Generator, Fatigue System, and workout auto-generation can be added later without reworking core workout data.
...
Represents a block of a workout. Examples: AMRAP, EMOM, For Time, Strength Sets. Attributes may include: segment type; duration; rest interval; ordered list of exercises.
