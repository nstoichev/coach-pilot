...
- [x] T063 [US4] Add `getWorkoutRequiredEquipment` in `src/services/workout-equipment.ts` per `specs/002-fitness-workout-spec/research.md` Decision 6; export from `src/services/index.ts`.
- [x] T064 [P] [US4] Add Vitest and `src/services/workout-equipment.test.ts` for trim, dedupe (case-insensitive), sort, and multi-segment aggregation; configure `vitest.config.ts` and exclude `**/*.test.ts` from `tsconfig.app.json`; add `npm run test` script in `package.json`.
- [x] T065 [US4] On Workout Board, render **Equipment required** below `boardContent` when `getWorkoutRequiredEquipment` is non-empty; add semantic token utilities in `src/ui/tw.ts` per **FR-031** and Contract 6 in `contracts/ui-contracts.md`.
